<template lang="pug">
.page(v-loading.fullscreen.lock="isWaiting")
  .modal
    h2.title Agrostream
    ...
    login() {
      ...
      Auth.login(this.username, this.pass)
        ...
        .catch(err => {
          ...
          this.$message({
            showClose: true,
            message: err === "INVALID" ? "Неверный E-mail или пароль" : err === "NO_RESPONSE" ? "Ошибка соединения с интернетом" : "Ошибка на сервере",
            type: "error"
          })
        })
    }
