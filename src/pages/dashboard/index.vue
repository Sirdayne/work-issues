      ...
      section.dashboard-section#dashboard-container
        .dashboard-part(v-if="cultures.length")
          ...
          .dashboard-container
            ...
            .el-slider-container
              el-slider(id="el-slider-1", v-model="value1", :show-tooltip="true", :max="maxWorksDates", :min="min", :format-tooltip="formatTooltipCultures")
              .el-slider-inner
                .el-slider-row(v-for="culture in firstCultures")
                  .el-slider-part( v-for="step in culture.fills", :style="oneStep", :class="step" )
...
export default {
  ...
  methods: {
    ...
    formatTooltipCultures(val) {
      if ((val != null) && (this.worksDates.length)) {
        this.elSliderDate1 = this.worksDates[val].daterus;
  ...
  computed: {
    cultures() {
      ...
      //заполняем фазы роста
      let worksDates = this.worksDates;
      //делаем пустой массив размером с worksDates
      let fills = Array(worksDates.length).fill('');

      let growthPhases = this.vuexGrowthPhases;

      culturesArray.forEach( x => x.fills = fills.slice() );

      culturesArray.forEach( x => {
        growthPhases.forEach( g => {

          if (x.id == g.cultureId){
            let firstIndex = 0;

            x.fills.fill('red', firstIndex, firstIndex + g.sproutPhase);
            firstIndex = firstIndex + g.sproutPhase;

            x.fills.fill('green', firstIndex, firstIndex + g.herringbonePhase);
            firstIndex = firstIndex + g.herringbonePhase;

            x.fills.fill('yellow', firstIndex, firstIndex + g.spikingPhase);
            firstIndex = firstIndex + g.spikingPhase;

            x.fills.fill('pur', firstIndex, firstIndex + g.spikingFlowering);
            firstIndex = firstIndex + g.spikingFlowering;

            x.fills.fill('blue-light', firstIndex, firstIndex + g.floweringRipenessPhase);
    ...
    oldWorks(){
      ...
      //достаем только suboperationId и оставляем только уникальные ID
      let areaAssignmentsUnique = this.vuexAreaAssignments.map(x => x.subOperationId).filter((v, i, a) => a.indexOf(v) === i);
    ...
    works(){
      ...
      if ((this.vuexCttops.length) && (this.vuexOperations.length)){
        ...
        let foundWorks = assigns.map( x => { x.name = ''; return x });

        assigns = foundWorks.map( x => {
    ...
    newWorks(){
      ...
      //делаем пустой массив размером с worksDates
      let fills = Array(worksDates.length).fill('');
      //заполняем свойства fills у assigns пустыми массивами
      newWorks.forEach( x => x.fills = fills.slice() );
      //если id из workTypeParameterPlanWorkTypesIds равен id у workingDates(это тоже workTypeParameterPlanWorkTypesIds) то заполням fills
      newWorks.forEach( x => {
        x.workTypeParameterPlanWorkTypesIds.forEach( w => {
    ...
    worksAreas(){
      ...
      let worksId = this.oldWorks.map(w => w.id);
    ...
    worksDonePlan() {
      ...
      this.vuexAreaAssignments.forEach( d => worksDonePlan += d.area);
...
//        this.vuexAreaAssignments.forEach(d => maxDate = (new Date(d.dateTimeRange.end) > maxDate) ? new Date(d.dateTimeRange.end) : maxDate);
      ...
      //minDate = new Date('2016-01-01T20:00:00')
      ...
      return {
        series:
          [{
            ...
            name: 'легенда 4',
            className: 'blue-dark-dim',
            meta: 'Meta Four'
          }, {
            value: this.pies[this.value1].pieFive,
            name: 'легенда 5',
            className: 'blue',
            meta: 'Meta Five'
          }]
