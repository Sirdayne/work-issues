          ...
          el-form(
            ...
            fields-select-plates(
              ...
              template(slot="item", slot-scope="props")
                div
                  b {{props.item.field.newName || props.item.field.name}}
            ...
            el-dialog(title="Процент обработки", v-model="sowingsPercentDialog", size="tiny")
              form(@submit.prevent="")
...
export default {
  ...
  computed: {
    ...
    sowings() {
      ...
      let sowings = []
      fromVuex("sowings").filter(sowing =>
  ...
  methods: {
    ...
    fetchCompleted() {
      ...
      this.unitTypes = fromVuex("unitTypes")
      this.brigades = fromVuex("brigades")
      this.cars = fromVuex("cars")
      this.carModels = fromVuex("carModels")
      this.cultures = fromVuex("cultures")
      this.fields = fromVuex("fields")
      this.fuelNorms = fromVuex("fuelNorms")
      this.vuexFuelNorms =  fromVuex("fuelNorms")
      this.instruments = fromVuex("instruments")
      this.organizationinstruments = fromVuex("organizationinstruments")
      this.technologyRecipe = fromVuex("technologyReciept")
      this.types = fromVuex("technologyRecieptTypes")
      this.works = fromVuex("works")
      this.workTypes = fromVuex("workTypes")
    ...
    updateWorks() {
      ...
      const organizationinstruments = createIndex(this.organizationinstruments, "instrumentId")
          ...
          works.filter(work => work.planWorkTypeId === workParamId).forEach(work => {
            norm.carModelsIds.forEach(carModelId => {
              norm.instrumentsIds.forEach(instrumentId => {
                ...
                const instrumentCount = organizationinstruments[instrumentId] ? organizationinstruments[instrumentId].count : 0
