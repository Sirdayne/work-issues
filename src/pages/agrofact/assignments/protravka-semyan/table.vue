...
export default {
  ...
  mounted() {
    EventBus.$on("WindowResize.windowInnerHeight", (windowInnerHeight) => {
      let maxHeight = windowInnerHeight - (60 + 10 + 18.72 + 21 + 18.72 + 287 + 10 + 36.25 + 10 + 32 + 10)
  ...
  methods: {
    ...
    filterAssignments(assignments) {
      return assignments.filter(a => {
        let from = !this.filterOptions.from || moment(a.dateTime, "YYYY-MM-DDTHH:mm:ss").isSameOrAfter(moment(this.filterOptions.from))
        let till = !this.filterOptions.till || moment(a.dateTime, "YYYY-MM-DDTHH:mm:ss").isSameOrBefore(moment(this.filterOptions.till))
        let date = from && till
        let cond = date
        return cond
      })
    ...
    initPagination() {
      this.page.size = +localStorage.getItem("assignments/seedmordant/pageSize") || 5
    ...
    handleSizeChange(num) {
      this.page.size = num
      localStorage.setItem("assignments/seedmordant/pageSize", +num)
      modByLib.addTooltips(this.paginate(this.tableData));
    },
  },
}
