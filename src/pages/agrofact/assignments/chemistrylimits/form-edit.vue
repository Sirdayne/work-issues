...
el-dialog.el-dialog--scrollable(:visible="formVisible", title="Редактирование", size="tiny", top="50px", :before-close="handleClose")
  el-form(:label-position="'top'", :model="item", :rules="rules", ref="ruleForm")
    ...
    template(v-if="isFieldWork")
      el-form-item.form-item-iterable(label="Поле")
        el-select(v-model="item.fieldId", filterable, clearable)
          el-option(v-for="f in filteredEditFields", :key="f.id", :label="f.newName", :value="f.id")
    template(v-else)
    ...
    template(v-if="isHerbicide")
      ...
      el-form-item(label="Количество сорняков")
...
export default {
  ...
  methods: {
    ...
    onWorkChange(workId) {
      ...
      this.setFilteredEditFields(this.itemClone.fieldId, cond)
      this.setFilteredEditSeedlimits(this.itemClone.seedLimitId, cond)
    },
    setFilteredEditFields(fieldId, cond) {
