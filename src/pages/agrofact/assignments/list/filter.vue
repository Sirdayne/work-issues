  ...
  el-form(label-width="90px")
    ...
    el-form-item(label="Статус")
      el-select.form-item-iterable(v-model="filterModel.statuses", filterable, multiple)
        el-option(v-for="s in filterModelData.processedstatuses", :label="s.name", :value="s.id", :key="s.id")
...
export default {
  ...
  methods: {
    initFilter() {
      ...
      this.filterModelData.processedstatuses = fromVuex("processedstatuses")
