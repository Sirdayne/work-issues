    ...
    el-table(
      ...
      el-table-column(
        ...
        template(slot-scope="scope")
          router-link(:to="`map?carId=${scope.row.carId}&date=${scope.row.formatDate}&tab=car-gps&tabNum=3`", target="_blank")
...
export default {
  ...
  computed: {
    stopformat() {
      ...
      let stopformat = []
      array.forEach(a => {
        let found = stopformat.findIndex(s => s.carId == a.carId && s.formatDate == a.formatDate)
        if (found == -1){
        ...
        array.forEach(a => {
          ...
          if (s.carId == stopjournal.carId && s.formatDate == stopjournal.formatDate){
            ...
            if (stopjournal.location){
              let found = s.geoZoneArray.findIndex(g => g == stopjournal.location)
              if (found == -1){
              ...
              if (field) {
                ...
                if (fieldName != "нет данных"){
                  let found = s.geoZoneArray.findIndex(g => g == fieldName)
                  if (found == -1){
              ...
              if (warehouse) {
                ...
                if (warehouseName != "нет данных"){
                  let found = s.geoZoneArray.findIndex(g => g == warehouseName)
                  if (found == -1){
