...
export default {
    ...
    "sliderStep"(val) {
      this.traktorTracks.forEach(t => {
        if (t.latLngTime[this.traktorTime[val]]){
          ...
          let popup = `Машина: ${t.carName}</br> Скорость: ${t.speedTime[this.traktorTime[val]]} км/ч</br> Время: ${moment(this.traktorTime[val], "YYYY-MM-DDTHH:mm:ss").format("DD/MM/YYYY HH:mm:ss")}</br>`
  ...
  methods: {
    removeTracks() {
      this.$store.dispatch("actionSetLegendAssignments", [])
      this.leafletTracks = []
      clearInterval(this.timerTraktor)
