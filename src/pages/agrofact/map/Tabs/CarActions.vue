...
export default {
  ...
  computed: {
    filteredStopjournals() {
      ...
      filteredStopjournals = this.stopjournals.filter(s => moment(s.start).format("L") == moment(this.mapDate).format("L"))
  ...
  created() {
    EventBus.$on("MapDateChanged", (date) => {
      this.mapDate = date
      this.getStopJournals(this.mapDate);
    })
    this.getStopJournals(this.mapDate);
    fetchEntities([
      "stoptypes",
  ...
  methods: {
    ...
    afterFetch() {
      this.stoptypes = fromVuex("stoptypes")
