  ...
  .fx-row-container(v-if="suboperationsTable.length > 0")
    .fx-row-container(v-for="suboperation in suboperationsTable")
      .fx-row
        ...
        .fx-cell(v-if="cols[getIdFromLabel('Выработка (га)')].active") {{suboperation.areaFix}}
        ...
        .fx-cell(v-if="cols[getIdFromLabel('% поля')].active") {{suboperation.percentOverall}}
...
export default {
  ...
  data() {
    return {
      ...
      loading: true,
      disabled: false,
      from: `${this.$root.context.year}-01-01T00:00:00`,
      to: `${this.$root.context.year}-12-31T23:59:59`,
    }
  },
  watch: {
    "id" () {
  ...
  methods: {
    ...
    filterAssignments() {
      ...
      this.groupSuboperationsFieldTable()
    },
    groupSuboperationsFieldTable() {
      this.closeSpoilers()
      ...
      const suboperationIds = [15, 23, 99, 51, 65, 94, 95, 96, 97, 140, 141]
      this.suboperationsTable.forEach(s => {
        let area = 0
        let seedlimitArea = 0
        this.seedlimits.forEach(seedlimit => {
          if (seedlimit.fieldId === s.fieldId)
            seedlimitArea += seedlimit.area
        })
        let field = this.fields.find(item => item.id === s.fieldId)
        if (suboperationIds.includes(s.id)) {
          area = seedlimitArea ? seedlimitArea : 100
        } else {
          area = field ? field.polygonArea : 100
        }
        s.percentOverall = (s.areaFix / area * 100).toFixed(2)
        s.areaFix = s.areaFix.toFixed(2)
      })
