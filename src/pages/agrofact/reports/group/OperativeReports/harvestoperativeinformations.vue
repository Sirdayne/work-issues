...
.operative-informations
  ...
  h2(class="tableHeading") Сводка по уборке
...
export default {
  ...
  data() {
    return {
      endpoint: "HarvestOperativeInformationsReport",
      xlsEndpoint: "HarvestOperativeInformations",
      xlsName: "Сводка по уборке"
    }
  },
  computed: {
    opinfos() {
          ...
          opinfo.forEach(item => {
            ...
            item.firstColStyle += item.operativeInformationType === 1 ? "background: #0300a1; color: #fff;" :
              item.operativeInformationType === 2 ? "background: #e3dc59;" :
                item.operativeInformationType === 3 ? "background: #5fe359;" :
                  item.operativeInformationType === 12 ? "background: #009930; color: #fff;" : ""
          })
          spoilerCount++
          array.push(opinfo)
        })
