...
.operative-informations
  ...
  h2(class="tableHeading") Сводка по сенокосу
...
export default {
  ...
  data() {
    return {
      endpoint: "HaymakingOperativeInformationsReport",
      xlsEndpoint: "HaymakingOperativeInformations",
      xlsName: "Сводка по сенокосу"
    }
  },
  computed: {
    opinfos() {
      ...
      if (this.operativeinformations.length > 0){
        this.operativeinformations.forEach(opinfo => {
          opinfo.reportLines.forEach(reportLine => {
