...
.operative-informations
  ...
  .loading-block(v-loading="loading", element-loading-text="Таблица загружается...")
    .fx-table(v-if="opinfos && opinfos.length > 0", style="position: relative;")
      ...
      .fx-container(v-if="operativeinformations.length > 0" v-for="opinfo in opinfos")
          ...
          .fx-container(v-for="(item, index) in reportLine")
            .fx-row(v-if="index === 0", @click="changeSpoiler(`.fx-spoiler-${item.spoiler}`)")
              .fx-cell(:style="item.firstColStyle") {{item.technologicalOperation}}
                span(v-if="reportLine.length > 1", style="margin-left: 5px; cursor: pointer;") +
...
export default {
  ...
  data() {
    return {
      endpoint: "ChemicalOperativeInformationsReport",
      xlsEndpoint: "ChemicalOperativeInformations",
      xlsName: "Сводка по химии"
    }
  },
  computed: {
    opinfos() {
      ...
      if (this.operativeinformations.length > 0){
        this.operativeinformations.forEach(c => {
          c.reportLines.forEach(reportLine => {
            reportLine.forEach(item => {
              item.spoiler = spoilerCount
              item.firstColStyle = this.firstColStyle
              item.firstColStyle += item.operativeInformationType == 3 ? "background: #5fe359;" :
                item.operativeInformationType == 1 ? "background: #0300a1; color: #fff;" :
                  item.operativeInformationType == 2 || item.operativeInformationType == 5 || item.operativeInformationType == 6 ? "background: #e3dc59;" : ""
            })
            spoilerCount++
          })
          array.push(c)
