      ...
      el-table-column(label="Вид сорняка", header-align="center")
        template(slot-scope="scope")
          template(v-for="item in scope.row.chemistryLimitWeedTypes")
            el-tag(:color="item.color") {{ item.weedName ? item.weedName : 'нет данных' }}
      el-table-column(label="Тип сорняка", header-align="center")
        template(slot-scope="scope")
          template(v-for="item in scope.row.chemistryLimitWeedTypes")
...
export default {
  ...
  methods: {
    ...
    setFields() {
      ...
      this.fields = fromVuex("fields").filter(x => this.fieldIds.indexOf(x.id) >= 0);
      this.setWCId()
