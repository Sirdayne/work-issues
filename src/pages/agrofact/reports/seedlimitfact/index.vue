...
div
  ...
  .el-table-cont
    el-table(
      ...
      el-table-column(prop="normDeviation", label="Отклонение по норме", header-align="center")
      el-table-column(prop="deviation", label="Отклонение итого", header-align="center")
...
export default {
  ...
  data() {
    return {
      ...
      filterModel: {
        ...
        culture: "",
        selectedDate: {
          from: moment().set({"year": this.$root.context.year, "month": 4, "date": 1}).startOf("day"),
  ...
  methods: {
    ...
    computeTableData() {
      let from = this.filterModel.selectedDate.from || new Date(moment().set({"year": this.$root.context.year, "month": 4, "date": 1}).startOf("day"))
      let till = this.filterModel.selectedDate.till || new Date(moment().set({"year": this.$root.context.year}).endOf("day"))
