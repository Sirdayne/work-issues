...
(function (factory) {
  ...
  L.Control.PolylineMeasure = L.Control.extend({
    ...
    _dragCircleMousemove: function (e2) {
      ...
      var lineNr = self._e1.target.cntLine;
      var circleNr = self._e1.target.cntCircle;
      self._e1.target.setLatLng(currentCircleCoords);
      self._lines[lineNr].points[circleNr] = currentCircleCoords;
      var lineCoords = self._lines[lineNr].path.getLatLngs(); // get Coords of each Point of the current Polyline
      if (circleNr >= 1) {   // redraw previous arc just if circle is not starting circle of polyline
        var newLineSegment1 = self._polylineArc(self._lines[lineNr].points[circleNr - 1], currentCircleCoords);
      ...
      if (circleNr < self._lines[lineNr].points.length - 1) {   // redraw following arc just if circle is not end circle of polyline
        var newLineSegment2 = self._polylineArc(currentCircleCoords, self._lines[lineNr].points[circleNr + 1]);
