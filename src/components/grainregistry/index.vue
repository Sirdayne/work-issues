...
div
  el-form(
    ...
    el-form-item(label="Поле", prop="fieldId")
      el-select(v-model="item.fieldId" clearable placeholder="Выбрать")
        el-option(
          v-for="f in fields",
          :key="f.id",
          :label="f.name",
    ...
    el-form-item(label="Культура", prop="cultureId")
      el-select(v-model="item.cultureId" clearable placeholder="Выбрать")
        el-option(
          v-for="c in cultures",
          :key="c.id",
          :label="c.name",
  ...
  el-table(
    ...
    el-table-column(
      prop="tractorDriver",
      label="ФИО водителя (тракториста)",
      header-align="center",
      width="180"
    )
    el-table-column(
...
<script>
  ...
  export default {
    ...
    computed: {
      fields() {
        let fields = this.fromVuex('fields')
        if (fields && fields.length) this.item.fieldId = fields[0].id
        return fields;
      },
      cultures() {
        let cultures = this.fromVuex('cultures')
