...
div
  ...
  h2(class="tableHeading") Журнал весовщика - отгрузки зерна
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      outgoingtransfers() {
        return this.fromVuex('outgoingtransfers')
      },
      totalItems: function() {
      ...
      tableData: function() {
        ...
        if (this.outgoingtransfers.length) this.loading = true
        let tableData = this.outgoingtransfers.filter(function(record) {
        ...
        if (this.outgoingtransfers.length) this.loading = false
        return tableData
      },
    },
    created() {
...
//      },
      ...
      exportTable(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            let endpoint        = "OutgoingTransferReport";
