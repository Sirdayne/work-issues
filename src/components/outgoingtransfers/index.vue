...
div
  ...
  h2 Журнал весовщика - отгрузки зерна
...
export default {
  ...
  computed: {
    tableData() {
      ...
      let tableData = this.outgoingtransfers.filter((record) => {
  ...
  methods: {
    afterFetch() {
      ...
      this.outgoingtransfers = this.fromVuex('outgoingtransfers')
    ...
    exportTable(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          let filename        = "Журнал весовщика(отгрузки зерна)";
          let body = {
