<template lang="pug">
  el-form-item(label="Время завершения")
    el-date-picker(
      v-model="model",
      type="datetime",
    ...
    methods: {
      ...
      initOnTransportationPage() {
        ...
        if (this.mode === 'Create') {
          ...
          this.model = moment(this.editable[this.modelName].end).format();
        }
      },
      handleChange(model) {
        this.emitEvents();
      ...
      emitEvents() {
        if (this.mode === 'Create') {
          EventBus.$emit(this.currentPage + '.CreateComponent.DateTimeEndFieldChanged', this.model);
        }
      },
      onAssignmentsPage(model) {
        const dateTimeRangeEnd    = moment(model).format("MM.DD.YYYY HH:mm");
        const dateTimeRangeStart  = this.handleDateTimeObjectInitialization('start');
