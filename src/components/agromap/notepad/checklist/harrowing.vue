...
div(v-loading="loading", element-loading-text="Загружается...")
  div
    h2 Данные чек-листов Боронование
    ...
    el-table(
      ...
      el-table-column(label="Глубина обработки", prop="depth", header-align="center")
      el-table-column(label="Влажность почвы", prop="soilMoisture", header-align="center")
      el-table-column(label="Направление обработки почвы по отношению к севу", prop="directionName", header-align="center")
...
<script>
  ...
  import update from 'components/agromap/notepad/checklist/update/harrowing'
  ...
  export default {
    ...
    methods: {
      afterFetch(){
        this.arrayCheckList = this.fromVuex('harrowingCheckList')
      ...
      deleteCheckList(id){
        this.deletedId = id
        this.endpointForDelete = 'harrowingCheckList/'
