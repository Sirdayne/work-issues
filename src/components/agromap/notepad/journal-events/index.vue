...
<script>
  ...
  import updatenote from 'components/agromap/notepad/notes/update'

  import updatepar from 'components/agromap/notepad/anketa/update/par'
  import updatesev from 'components/agromap/notepad/anketa/update/sev'
  import updateuborka from 'components/agromap/notepad/anketa/update/uborka'
  import updateuhod from 'components/agromap/notepad/anketa/update/uhod'
  import updatevpr from 'components/agromap/notepad/anketa/update/vpr'

  import updatechemical from 'components/agromap/notepad/checklist/update/chemical'
  import updatefertilizer from 'components/agromap/notepad/checklist/update/fertilizer'
  import updateharrowing from 'components/agromap/notepad/checklist/update/harrowing'
  import updateharvest from 'components/agromap/notepad/checklist/update/harvest'
  import updatesowing from 'components/agromap/notepad/checklist/update/sowing'

  import viewnote from 'components/agromap/notepad/notes/view'
  ...
  export default {
    ...
    methods: {
      afterFetch(){
        this.eventlogs = this.fromVuex('eventlogs')
        this.notes = this.fromVuex('notes')
        this.fieldSurveys = this.fromVuex('fieldSurveys')
        this.chemicalCheckList = this.fromVuex('chemicalCheckList')
        this.harrowingCheckList = this.fromVuex('harrowingCheckList')
        this.fertilizerCheckList = this.fromVuex('fertilizerCheckList')
        this.sowingCheckList = this.fromVuex('sowingCheckList')
        this.harvestCheckList = this.fromVuex('harvestCheckList')
        this.loading = false
      },
      edit(eventId) {
        let finded = this.eventlogs.find(e => e.id == eventId)
        if (finded){
          if (finded.noteId){
            this.form = this.notes.find(n => n.id == finded.noteId)
            this.dialogNote = true
          } else if (finded.fieldSurveyId) {
            this.form = this.fieldSurveys.find(n => n.id == finded.fieldSurveyId)
          ...
          } else if (finded.checkListId) {
            if (finded.checkListType == 1) {
              this.form = this.chemicalCheckList.find(item => item.id == finded.checkListId)
              this.dialogChemical = true
            }
            if (finded.checkListType == 2) {
              this.form = this.fertilizerCheckList.find(item => item.id == finded.checkListId)
              this.dialogFertilizer = true
            }
            if (finded.checkListType == 3) {
              this.form = this.sowingCheckList.find(item => item.id == finded.checkListId)
              this.dialogSowing = true
            }
            if (finded.checkListType == 4) {
              this.form = this.harvestCheckList.find(item => item.id == finded.checkListId)
              this.dialogHarvest = true
            }
            if (finded.checkListType == 5) {
              this.form = this.harrowingCheckList.find(item => item.id == finded.checkListId)
