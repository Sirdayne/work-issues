...
div
  ...
  h2(class="tableHeading") Журнал весовщика - прием
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      incomingtransfers() {
        return this.fromVuex('incomingtransfers')
      },
      totalItems: function() {
      ...
      tableData: function() {
        ...
        if (this.incomingtransfers.length) this.loading = true
        let tableData = this.incomingtransfers.filter(function(record) {
        ...
        if (this.incomingtransfers.length) this.loading = false
        return tableData
      },
    },
    created() {
      ...
      exportTable(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            let filename        = "Журнал весовщика(прием)";
