...
div
  ...
  el-form(:inline="true", :label-position="'top'", :model="item", :rules="rules", ref="ruleForm")
    ...
    el-form-item.form-item-iterable(label="Работа")
      el-select(v-model="item.workTypeParameterPlanWorkTypeId", filterable, clearable)
        el-option(v-for="w in works", :label="w.name", :value="w.id", :key="w.id")
    el-form-item.form-item-iterable(label="Посев")
      el-select(v-model="item.seedLimitId", @change="extractExtraInfoFromSeedlimit", filterable, clearable)
...
export default {
  ...
  methods: {
    afterFetch() {
      ...
      this.works = this.fromVuex('works').filter(w => w.isMacrofertilizer)
    ...
    save() {
      ...
      http[httpMethod](`chemistrylimits/${this.$root.context.organization}`, body)
        .then(() => {
          ...
          this.item = {chemistryChemicalTreatments: [{}]}
          this.refresh()
        })
        .catch(() => {
          this.loadingItem.save = false
    ...
    getLZK() {
      ...
      body.date = moment(body.date).format("YYYY-MM-DDTHH:mm:ss")
      let filename, api
      if (this.item.workTypeParameterPlanWorkTypeId == 65)  {
        api = "SeedMineralForm"
